import asyncio
import logging
import json
import aio_pika
import sqlite3
from datetime import datetime
from typing import Dict
from app.core.model_consumer.model_processing import ModelProcessing
#from app.config import settings
from app.core.model_consumer.model import load_model 
from app.core.notification_consumer.teams_integration import TeamsIntegration,EmailData, TriggerMatcher
# Setup basic logging
from app.core.model_consumer.mentenance_check import MaintenanceWindowService
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
from app.core.notification_consumer.jira_integration import JiraIntegration
from app.config import settings
from app.core.model_consumer.producer import send_data_to_queue
from app.db_functions.db_functions import *
from app.db_functions.db_schema2 import *
from app.logging.logging_config import notification_logger
from app.logging.logging_decorator import log_function_call

#from app.api.v1.endpoints.api import app_executor
# Global RabbitMQ URL reference (defined in main but needed here for publishing retry)
# We must define a placeholder here, it will be populated by main in a real application
RABBITMQ_URL_GLOBAL ="amqp://guest:guest@localhost/" #settings.RABBITMQ_URL

RABBITMQ_URL = settings.RABBITMQ_URL
JIRA_QUEUE_NAME = settings.JIRA_QUEUE_NAME
JIRA_DLQ_NAME = settings.JIRA_DLQ_NAME

# ==============================
# Dedicated Publish Function for Retry (Indentation Fixed)
# ==============================
@log_function_call(notification_logger)
async def publish_retry_message(
    body: bytes, routing_key: str, headers: Dict, url: str = RABBITMQ_URL_GLOBAL
):
    """Establishes an isolated connection/channel to republish the message."""
    try:
        connection = await aio_pika.connect_robust(url)
        async with connection:
            channel = await connection.channel()

            # --- FIX: Instantiate the Default Exchange object directly (name="") ---
            # This avoids the network conflict caused by trying to DECLARE the built-in exchange.
            default_exchange = aio_pika.Exchange(
                channel=channel,
                name="",
                type=aio_pika.ExchangeType.DIRECT,
                durable=True
            )
            # --- END FIX ---

            message_to_send = aio_pika.Message(
                body=body,
                headers=headers,
                delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
            )

            await default_exchange.publish(
                message_to_send,
                routing_key=routing_key,
            )
            print(f"Successfully republished message for retry to '{routing_key}'")

    except Exception as e:
        logging.error(f"FATAL: Failed to republish message for retry: {e}")
        # If publishing fails here, the message is permanently lost, hence the importance of robust publish


# ==============================
# Consumer with DLQ (Indentation Fixed)
# ==============================
MAX_RETRIES = 5

@log_function_call(notification_logger)
async def process_message(message: aio_pika.IncomingMessage, dlx_exchange: aio_pika.Exchange,model,tokenizer,app_executor):
    # This context manager manages the lifecycle of the message on the consumption channel

    try:
        loop = asyncio.get_running_loop()
        body = json.loads(message.body.decode())
        # mp = ModelProcessing(model,tokenizer)
        # summary= await loop.run_in_executor(app_executor, mp.summary,body)
        output=body
        # await message.ack()
        #summary="test summary"
        # output["body"]=body["content"]
        # output["subject"]=body["subject"]
        # output['sender'] = body["sender_address"]
        # output['timestamp'] = body['received_time']
        # output['generated_summary']=body['summary']
        # output["path"]=body["msg_path"]
        jira=JiraIntegration()
        print(output)
        db = next(get_db())
        query = db.query(JiraEntry).filter(JiraEntry.email_id==output.get('email_id')).first()
        #status = query.status if query else False
        db.close()
        if query:
            print(" a row already exist ,checking if it sent to teams")
            row={"key":query.jiraticket_id,"assignee_name":query.assigned_to,"teams_flag":query.teams_flag,"team":query.teams_channel}
            if row["teams_flag"]==True:
                await message.ack()
                return
            else:
                key= row["key"]
                assignee_name=row["assignee_name"]
                category=None
                team=row["team"]
        else:
            trigger_matcher = TriggerMatcher()
            team, confidence, matched_trigger, responsible_person = trigger_matcher.find_best_match(output.get('trigger_name', ''))
            print(team, confidence, matched_trigger, responsible_person)
            db=next(get_db())
            row=db.query(TriggerMapping).filter(TriggerMapping.trigger_name==matched_trigger).first()
            responsible_person=row.responsible_persons
            print(responsible_person)
            # Use responsible_person for Jira assignment (fallback to default if None)
            not_responsible_person=["yves.maennig@bitzer.de",
    "jurand.wolff@bitzer.de",
    "marc-oliver.thews@bitzer.de",
    "thomas.wyrtki@bitzer.de",
    "rainer.schaaf@bitzer.de",
    "jalil.samimi@bitzer.de",
    "victor.restivo@bitzer.de",
    "juergen.kraiss@bitzer.de",
    "reinhold.gellert@bitzer.de"]
            if responsible_person in not_responsible_person or responsible_person is None:
                responsible_person="simon.schmid@bitzer.de"
            print(responsible_person)
            
            assignee_email = responsible_person 
            
            # ✅ FIXED: Create JIRA ticket - returns (None, None, None, None) if duplicate detected
            result = await asyncio.to_thread(
                jira.create_ticket_sync, output, assignee_email
            )
            
            # Handle duplicate case - ticket creation was skipped
            if result is None or result[0] is None:
                notification_logger.info(f"✅ Duplicate detected for trigger '{output.get('trigger_name')}' + resource '{output.get('resource_name')}' - ticket skipped")
                await message.ack()
                notification_logger.info("✅ Message acknowledged - duplicate handled successfully")
                return  # Exit early - don't continue processing
            
            # Unpack the result only if we have a valid ticket
            key, machine_name, category, assignee_name = result

            # Set Team field after ticket creation
            if key and team:
                await asyncio.to_thread(jira.set_team_field, key, team)
            db=next(get_db())
            data={}
            data['jiraticket_id']=key
            data['assigned_to']=responsible_person
            data['created_at'] = datetime.strptime(datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[0:23], '%Y-%m-%d %H:%M:%S.%f')
            data['teams_flag'] = 'false'
            row = insert_jira_entry(db,output.get('email_id'),data)
            print("inserted in the jira in consumer_notification",row)
            print(key, machine_name, category, assignee_name)
            db.close()
            #Initializing teams 
            #assignee_name="Team"
            

        teams = TeamsIntegration()
        output["resource_name"]=output["resource_name"].split("@")[0] if '@' in output["resource_name"] else output["resource_name"].split(".")[0]

        output2 = {'subject':output['subject'],'sender':output['sender'],'body':output['body'],'priority':output['priority'],'trigger_name':output['trigger_name'],'timestamp':output['timestamp']}
        #output2=None


        email_data = EmailData.from_dict(output2)

        if teams:
                db=next(get_db())
                teams_success = await teams.send_notification(
                    email_data, key, output["resource_name"], category, assignee_name
                )
                if teams_success:
                  print("teams_sucess worked ")
                  stmt = update(JiraEntry).where(JiraEntry.jiraticket_id == key).values(
        teams_flag='true',
        teams_channel=team  # 'team' variable already exists from trigger_matcher.find_best_match()
    )
                  db.execute(stmt)
                  db.commit()
                  db.close()
                
        else:
            print(f"   âŒ Teams connection error and teams notification creation failed")
    
        await message.ack()
        print("message acked")
                

        
        
        #logging.info("Received: %s", body)
        
            
        # Pipeline
        #parsed = await parse_message(body)
        #rules_output = await rules_engine(parsed)
        #await jira_function(rules_output)
        #print(f"done {rules_output}")

    except Exception as e:
        # Retry tracking in headers
        headers = message.headers or {}
        retries = headers.get("x-retries", 0)
        print(e)
        if retries < MAX_RETRIES:
            # Log the retry count being attempted
            logging.warning("Retry %s/%s for message %s", retries + 1, MAX_RETRIES, body)
            print(f"retrying and setting header to {retries + 1} time")

            # 1. NACK the current message delivery (DO NOT requeue)
            # This clears the message from the current consumption channel.
            await message.nack(requeue=False)

            # 2. Prepare NEW headers by copying existing ones
            new_headers = message.headers.copy() if message.headers else {}
            new_headers["x-retries"] = retries + 1

            # --- FIX: Delegate publishing to an isolated function/channel ---
            # This prevents the race condition and "channel closed" errors.
            await publish_retry_message(
                message.body,
                message.routing_key,
                new_headers
            )

        else:
            logging.error("Max retries reached. Sending to DLQ & DB...")
            await message.nack(requeue=False)

            # Send to DLQ using the dedicated DLX
            await dlx_exchange.publish(
                aio_pika.Message(
                    body=message.body,
                    headers={"x-error": str(e)},
                    delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
                ),
                routing_key="dlq.jira",
            )
            print(f"DLQ done.")
            # Log to DB (uncommented for full functionality)
            #log_error_to_db(message.body.decode(), str(e))
        


# ==============================
# Main loop (Indentation Fixed)
# ==============================
@log_function_call(notification_logger)
async def consumer_main(app_executor,model,tokenizer):
    global RABBITMQ_URL_GLOBAL # Use global for the URL so the isolated function can access it

    # Setup logging again (redundant but ensures configuration)
    logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
    #init_db()

    #RABBITMQ_URL = "amqp://guest:guest@localhost/"
    # RABBITMQ_URL_GLOBAL = "amqp://guest:guest@localhost/" #settings.RABBITMQ_URL # Set the global variable for isolated publishing
    # QUEUE_NAME = "my_async_queue"#settings.QUEUE_NAME
    DLQ_NAME = "my_async_queue_dlq"
    #global pipe 
    # model,tokenizer = await asyncio.to_thread(load_model)
    connection = await aio_pika.connect_robust(RABBITMQ_URL,heartbeat=600 ,timeout=6000)
    async with connection:
        channel = await connection.channel()
        await channel.set_qos(prefetch_count=1)
        # Declare normal queue
        queue = await channel.declare_queue(JIRA_QUEUE_NAME, durable=True)

        # Declare Dead Letter Queue & Exchange
        dlx_exchange = await channel.declare_exchange("dlx", aio_pika.ExchangeType.DIRECT, durable=True)
        dlq = await channel.declare_queue(JIRA_DLQ_NAME, durable=True)
        await dlq.bind(dlx_exchange, routing_key="dlq.jira")

        logging.info("Consumer started. Waiting for messages in '%s'...", JIRA_QUEUE_NAME)
        print(f"starting")
        # Consume messages
        await queue.consume(lambda msg: process_message(msg, dlx_exchange,model,tokenizer,app_executor), no_ack=False)

        # Keep running
        await asyncio.Future()
    

# Check if the script is run directly
# if __name__ == "__main__":
#     try:
#         asyncio.run(consumer_main())
#     except KeyboardInterrupt:
#         logging.info("Consumer stopped by user.")